	...
	<View style={ELEMENT_STYLE}>
		...
		<View style={tw("flex-col")}>
			{u.contactInfo.items.map((contact, i) =>
				<View style={tw("flex-row gap-2")}>
					<View key={contact.id} style={tw("bg-accent border-b border-gray-100 items-center justify-center p-1 " + (i == 0 ? 'rounded-t-full' : '') + (i == u.contactInfo.items.length - 1 ? 'rounded-b-full' : ''))}>
						{{
	...
	return (
		...
		>
			<Page size="A4" style={tw("px-10 text-gray-700")}>
				<View style={tw("flex-row")}>
					<View style={tw("w-[35%] py-10 flex-col min-h-screen bg-secondary")}>
						<View>
							<View style={tw("border-b border-accent flex items-center pb-10")}>
